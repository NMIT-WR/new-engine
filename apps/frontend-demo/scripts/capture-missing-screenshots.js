  ...
  try {
    ...
    await page.screenshot({
      path: path.join(
        'screenshots',
        'ui-check',
        'product-detail-mobile-light.png'
      ),
      fullPage: false,
    })
      ...
      for (const theme of ['light', 'dark']) {
        ...
        const added = await page.evaluate(() => {
          ...
          const addButton = buttons.find((btn) => {
            ...
            return (
              text.includes('add to cart') || ariaLabel.includes('add to cart')
            )
          })
        ...
        if (!added) {
          ...
          await page.goto(
            'http://localhost:3000/products/classic-cotton-t-shirt',
            {
              waitUntil: 'domcontentloaded',
              timeout: 10000,
            }
...
async function fixDarkThemeScreenshots() {
  ...
  const pagesToFix = [
    ...
    {
      name: 'product-detail',
      path: '/products/classic-cotton-t-shirt',
      breakpoints: ['mobile', 'tablet', 'desktop'],
    },
  ]
    ...
    for (const pageConfig of pagesToFix) {
      for (const breakpoint of pageConfig.breakpoints) {
        ...
        if (toggled) {
          ...
          console.log(
            `⚠ Could not find theme toggle for ${pageConfig.name}-${breakpoint}`
          )
        }
