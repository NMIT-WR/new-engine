...
(async () => {
  ...
  try {
    ...
    console.log('   Redirected to home:', currentUrl === 'http://localhost:3000/' ? 'Yes ✓' : 'No ✗');
    ...
    try {
      ...
      const userElement = await page.$('header button, header [aria-label*="account"], header [aria-label*="user"]');
      if (userElement) {
        const userText = await page.evaluate(el => el.textContent, userElement);
    ...
    if (header) {
      await header.screenshot({ path: 'screenshots/test-login-4-header-detail.png' });
