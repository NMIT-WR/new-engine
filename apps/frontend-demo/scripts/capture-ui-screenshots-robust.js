...
async function captureScreenshot(browser, pageConfig, breakpoint, theme) {
  ...
  try {
    ...
    if (theme === 'dark') {
      try {
        ...
        const themeToggleFound = await page.evaluate(() => {
          ...
          const themeButton = buttons.find(btn => {
            ...
            return (ariaLabel && ariaLabel.toLowerCase().includes('theme')) ||
                   ...
                   (svg && (svg.innerHTML.includes('sun') || svg.innerHTML.includes('moon')));
          });
        ...
        if (themeToggleFound) {
          ...
          console.log(`  Warning: Could not find theme toggle for ${pageConfig.name}-${breakpoint.name}`);
        }
      } catch (error) {
        console.log(`  Warning: Theme toggle failed for ${pageConfig.name}-${breakpoint.name}:`, error.message);
      }
    }
    ...
    console.error(`  ✗ Failed ${pageConfig.name}-${breakpoint.name}-${theme}:`, error.message);
  } finally {
    await page.close();
  }
}
...
async function captureAllScreenshots() {
    ...
    for (const breakpoint of breakpoints) {
      for (const theme of ['light', 'dark']) {
        try {
          ...
          results.failed.push(`${pageConfig.name}-${breakpoint.name}-${theme}: ${error.message}`);
        }
      }
    }
  }
  ...
  const capturedFiles = fs.readdirSync(path.join('screenshots', 'ui-check'))
