...
async function testThemeToggle() {
  ...
  try {
    ...
    await page.waitForSelector('label input[type="checkbox"]', { timeout: 5000 });
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    try {
      ...
      const clicked = await page.evaluate(() => {
        ...
        const labels = Array.from(document.querySelectorAll('label'));
        const switchLabel = labels.find(label => label.querySelector('input[type="checkbox"]'));
        if (switchLabel) {
          switchLabel.click();
          return 'label';
        }
  ...
  console.log(`Persistence: ${themeBeforeRefresh === themeAfterRefresh ? '✅ WORKING' : '❌ NOT WORKING'}`);
  ...
  console.log(`LocalStorage persistence: ${hasLocalStorage ? '✅ WORKING' : '❌ NOT WORKING'}`);
