...
import { Tabs, type TabItem } from 'ui/src/atoms/tabs'
...
const productTabsStyles = tv({
  slots: {
    ...
    tabText: 'text-product-tabs-content-size text-product-tabs-content-fg leading-relaxed',
    heading: 'text-product-tabs-heading-size font-product-tabs-heading text-product-tabs-heading-fg',
    featuresList: 'space-y-product-tabs-features-gap pl-5',
    featureItem: 'text-product-tabs-content-size text-product-tabs-content-fg list-disc',
    specList: 'space-y-product-tabs-spec-gap',
    specRow: 'flex gap-product-tabs-spec-gap py-product-tabs-table-cell-y border-b border-product-tabs-spec-border last:border-0',
    specLabel: 'font-product-tabs-spec-label text-product-tabs-spec-size text-product-tabs-spec-label min-w-[var(--spacing-product-tabs-spec-label-width)]',
    specValue: 'text-product-tabs-spec-size text-product-tabs-spec-value',
    accordionContent: 'text-product-tabs-content-size text-product-tabs-content-fg space-y-product-tabs-features-gap',
    ...
    reviewSummary: 'bg-product-tabs-review-bg p-product-tabs-review-card rounded-product-tabs-review-bg',
    reviewSummaryContent: 'flex items-center gap-product-tabs-review-rating-gap',
    reviewList: 'space-y-product-tabs-content-gap',
    reviewCard: 'border-b border-product-tabs-review-border pb-product-tabs-content-gap',
    ...
    reviewComment: 'text-product-tabs-content-size text-product-tabs-content-fg leading-relaxed',
    noReviews: 'text-center py-product-tabs-review-card bg-product-tabs-review-bg rounded-product-tabs-review-bg',
  },
})
              ...
              <dl className={specList()}>
                {product.specifications.map((spec, idx) => (
                  <div key={idx} className={specRow()}>
                    <dt className={specLabel()}>
                      {spec.name}
                    </dt>
                    <dd className={specValue()}>
                      {spec.value}
                    </dd>
                  </div>
                ))}
                ...
                },
                {
                  ...
                  content: (
                    <div className={accordionContent()}>
                      ...
                      <p className={`font-product-tabs-spec-label mt-product-tabs-spec-gap`}>Size Chart:</p>
                      <table className={sizeTable()}>
                        <thead>
                          <tr className={sizeTableRow()}>
                            <th className={`text-left ${sizeTableCell()}`}>Size</th>
                            <th className={`text-left ${sizeTableCell()}`}>EU</th>
                            <th className={`text-left ${sizeTableCell()}`}>US</th>
                            <th className={`text-left ${sizeTableCell()}`}>UK</th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr className={sizeTableRow()}>
                ...
                <div key={review.id} className={reviewCard()}>
                  <div className={reviewHeader()}>
                    <div className="space-y-1">
                      <div className={`flex items-center gap-product-tabs-spec-gap`}>
                        <Rating value={review.rating} readOnly />
                        <h5 className={`${specLabel().replace('min-w-[var(--spacing-product-tabs-spec-label-width)]', '')}`}>
                          {review.title}
                        </h5>
                        {review.verified && (
