...
**Naming:** Directory names use hyphens (`my-module/`), module keys use underscores (`my_module`). Key constraint: alphanumeric + underscores only.
...
**Why this works:** Provider services are lazy singletons. Constructor runs at request time, when all modules are loaded.

**Tip:** Export the module key as a constant (`MY_CLIENT_MODULE = "my_client"`) and use it everywhere to avoid hardcoding the name.
...
**Watch out:** Use `Modules.CACHING` (not `CACHE`), `Modules.LOCKING` (not `LOCK`). The constants use gerund form (-ING).
...
if (obj.prop) { /* use obj.prop */ }  // ✅ Use type guards
```

**Functional core, imperative shell:**
Extract pure functions (no side effects, no container dependencies) to separate files. This allows importing and testing them without loading modules that have runtime dependencies (OAuth clients, service instantiation). Keep side-effect code (API calls, DB queries, container resolution) in the main file.
  ...
  routes: [
    {
      ...
      middlewares: [validateAndTransformQuery(Schema, { defaults: ["id"], allowed: ["id", "title"], isList: true })],
    },
    {
      methods: ["POST"],
      matcher: "/store/my-entities",
...
**Multi-container:** For shared state (tokens, rate limiting), always prioritize Redis over local variables. Local fallback only when Redis unavailable.
...
**Tip:** Keep both short and identical when you have one instance: `identifier = "carrier"`, `id = "carrier"` → `carrier_carrier`
...
**⚠️ Job tests:** Must be in `tests/unit/jobs/`, NOT `src/jobs/__tests__/`. Medusa loads all files in `src/jobs/` at runtime—test files with Jest globals (`describe`, `jest`) will crash the dev server.
...
- Routes calling external APIs (PPL, payment gateways) — requires mocking external service, defeating integration purpose
...
**When to use:** Custom methods on MedusaService subclasses that can't be properly unit tested by mocking inherited methods. If you find yourself mocking `listMyEntities`, `createMyEntities`, etc., use module integration test instead.
...
- After each session, ask:
  ...
  > "[Backend] Based on our conversation and Claude Code best practices, is there anything that should be added to `CLAUDE.MD`, or does anything make current instructions obsolete?"
...
**IMPORTANT:** All code examples in this file must use generic names (`my_module`, `my_provider`, `MyService`, etc.), never project-specific names. This keeps examples reusable and prevents coupling documentation to specific implementations.