import {IFulfillmentModuleService, Logger, RuleOperatorType, WorkflowTypes} from "@medusajs/framework/types"
import {ContainerRegistrationKeys, Modules} from "@medusajs/framework/utils"
import {createStep, StepResponse,} from "@medusajs/framework/workflows-sdk"
import {createShippingOptionsWorkflow, updateShippingOptionsWorkflow, updateShippingOptionTypesWorkflow} from "@medusajs/medusa/core-flows"
...
export const createShippingOptionsStep = createStep(CreateShippingOptionsStepId, async (
    ...
    const missingOptions = input.filter(i => !existingOptions.find(j => j.name === i.name))
    const updateOptions = input.flatMap(inputOption => {
        const existingOption = existingOptions.find(existing => existing.name === inputOption.name)
        if (existingOption) {
            return [{
    ...
    if (missingOptions.length !== 0) {
        ...
        const {result: createResult} = await createShippingOptionsWorkflow(container).run({
            input: workflowInput
        })
    ...
    if (updateOptions.length !== 0) {
        ...
        const updateInputs = updateOptions.map(({existing, input: inputOption}) => {
        ...
        const {result: updateResult} = await updateShippingOptionsWorkflow(container).run({
            input: updateInputs
        })
        ...
        const typesToUpdate = updateOptions
            .filter(({existing, input: inputOption}) => existing.type?.code === inputOption.type.code && existing.type)
        ...
        for (const typeToUpdate of typesToUpdate) {
            logger.info(`Updating existing shipping option type: ${typeToUpdate.update.code}`)
