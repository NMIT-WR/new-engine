...
export const createStockLocationSeedStep = createStep(CreateStockLocationStepId, async (
    input: CreateStockLocationStepInput,
    {container}
) => {
    ...
    if (updateStockLocations.length !== 0) {
        ...
        for (const stockLocationToUpdate of toUpdate) {
            ...
            const updateResultFixed: StockLocationDTO = updateResultUnknown as StockLocationDTO
