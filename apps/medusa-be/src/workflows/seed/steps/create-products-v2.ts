...
type ProductInput = {
  ...
  variants?: {
    ...
    ean?: string
    material?: string
    weight?: number // Added: variant weight in grams
    length?: number // Added: variant length in mm
    width?: number // Added: variant width in mm
    height?: number // Added: variant height in mm
    options?: {
      [key: string]: string
    }
    images?: {
  ...
  metadata?: any // Added: product-level metadata
}
...
export type CreateProductsV2StepInput = ProductInput[]

const CreateProductsV2StepId = "create-products-v2-seed-step"
...
export const createProductsV2Step = createStep(
  CreateProductsV2StepId,
  async (input: CreateProductsV2StepInput, { container }) => {
    const result: string[] = []
      ...
      return [
        {
          ...
          metadata: inputProduct.metadata, // Added: product metadata
          variants: inputProduct.variants?.map((inputVariant) => {
            ...
            return existingVariant
              ? {
                  ...
                  weight: inputVariant.weight, // Added: variant weight
                  length: inputVariant.length, // Added: variant length
                  width: inputVariant.width, // Added: variant width
                  height: inputVariant.height, // Added: variant height
    ...
    if (missingProducts.length !== 0) {
      ...
      const createProducts = missingProducts.map((p) => {
        ...
        return {
          ...
          images: p.images ?? [],
          options: p.options,
          metadata: p.metadata, // Added: product metadata
          variants: p.variants?.map((v) => ({
            title: v.title,
            sku: v.sku,
            ean: v.ean,
            material: v.material,
            weight: v.weight, // Added: variant weight
            length: v.length, // Added: variant length
            width: v.width, // Added: variant width
            height: v.height, // Added: variant height
            options: v.options,
