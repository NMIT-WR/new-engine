...
export const createProductCategoriesStep = createStep(CreateProductCategoriesStepId, async (
    ...
    const updateParentProductCategories = input.filter(i => i.parentHandle !== undefined && i.parentHandle !== null).map(i => {

        const category = allProductCategories.find(j => j.handle === i.handle)
        const parent = allProductCategories.find(j => j.handle === i.parentHandle)

        if (category === undefined || parent === undefined) {
            console.log(i)
            throw new Error(`Could not find category parent pair ${i.handle} -> ${i.parentHandle}`)
        }
