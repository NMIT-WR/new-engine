...
export const syncMeilisearchProducersStep = createStep(
    ...
    async ({filters, limit, offset}: SyncMeilisearchProducersStepInput, {container}) => {
        ...
        const producersToDelete = Array.from(existingProducerIds).filter((id) => !producers.some((c) => c.id === id))
        ...
        await Promise.all(producerIndexes.map((index) => meilisearchService.deleteDocuments(index, producersToDelete)))
