...
class ProducerModuleService extends MedusaService({
  ...
  async upsertProducer(
    ...
    const attributeTypesExisting = await this.listProducerAttributeTypes({
      name: { $in: attributeTypes },
    })
    const attributeTypesToCreate = attributeTypes.filter(
      (i) => !attributeTypesExisting.find((j) => j.name === i)
    )
    ...
    const attributesExisting = await this.listProducerAttributes({
      producer_id: producer.id,
      attribute_type_id: {
        $in: attributeTypes.map((i) => attributeTypesMap.get(i)),
      },
    })
    ...
    const attributesToCreate = attributes
      .filter(
        (i) =>
          !attributesExisting.find(
            (j) => j.attributeType.id === i.attributeType_id
          )
      )
      .map((i) => ({
        producer_id: producer.id,
    ...
    const attributesToUpdate = attributesExisting
      .map((attributeExisting) => {
        const attribute = attributes.find(
          (i) => i.attributeType_id === attributeExisting.attributeType.id
        )
        if (!attribute) {
          return null
        }
