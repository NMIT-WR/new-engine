...
import seedNorthern2Workflow, {
  type SeedNorthern2WorkflowInput,
} from "../workflows/seed/workflows/seed-northern-2"
...
const CACHE_KEYS = {
  EXCEL_DATA: "seed-northern-2:excel-data",
} as const
    ...
    variants: [
      {
        ...
        prices,
        weight: variantWeight, // FIXED: now properly set
        length: variantLength, // FIXED: now properly set
        width: variantWidth, // FIXED: now properly set
        height: variantHeight, // FIXED: now properly set
...
export default async function seedNorthern2({ container }: ExecArgs) {
  const logger = container.resolve<Logger>(ContainerRegistrationKeys.LOGGER)

  logger.info("Starting Northern seed v2 with full store setup...")
  ...
  const input: Omit<SeedNorthern2WorkflowInput, "categories" | "products"> = {
    salesChannels: [
      {
  ...
  logger.info(`Categories: ${categories.map((c) => c.title).join(", ")}`)
  ...
  logger.info("Running seed workflow v2...")
  const { result } = await seedNorthern2Workflow(container).run({
    input: { ...input, categories, products: transformedProducts },
  })

  logger.info("Northern seed v2 completed successfully")
