...
function convertToMedusaProducts(
  ...
  return products.map(product => {
    ...
    return {
      ...
      category_ids: product.category_slug && categoryMap[product.category_slug]
      ...
      variants: [
        {
          ...
          prices: [
            {
              amount: parseInt(product.product_price * 100), // Medusa expects prices in cents
              currency_code: "eur",
            },
            {
              amount: parseInt(product.product_price * 100 * 1.1), // Simple USD conversion
              currency_code: "usd",
            },
          ],
        },
...
function extractCategories(products: ProductRecord[]): {
  ...
  const categoriesMap: Record<string, { slug: string; name: string; image_url?: string }> = {};
...
export default async function seedProductsFromDb({ container }: ExecArgs) {
  ...
  const existingCategoryMap = await checkExistingCategories(container, categoryHandles);

  // Filter out categories that already exist
  const newCategories = categories.filter(category => !existingCategoryMap[category.slug]);
  logger.info(`Found ${Object.keys(existingCategoryMap).length} existing categories, creating ${newCategories.length} new categories`);
  ...
  const productCategories = newCategories.map(category => ({
    ...
    is_active: true,
    is_internal: false,
    metadata: category.image_url ? { image_url: category.image_url } : undefined,
  }));
  ...
  const existingCollectionMap = await checkExistingCollections(container, collectionHandles);

  // Filter out collections that already exist
  const newCollections = collections.filter(collection => !existingCollectionMap[collection.handle]);
  logger.info(`Found ${Object.keys(existingCollectionMap).length} existing collections, creating ${newCollections.length} new collections`);
  ...
  if (newCollections.length > 0) {
    ...
    logger.info(`Successfully created ${collectionResult.length} new collections`);
  } else {
  ...
  while (hasMore) {
    try {
      ...
      logger.info(`Checking for existing products with ${productHandles.length} handles...`);
      const existingProductMap = await checkExistingProducts(container, productHandles);

      // Filter out products that already exist
      const newProducts = medusaProducts.filter(product => !existingProductMap[product.handle]);
      logger.info(`Found ${Object.keys(existingProductMap).length} existing products, creating ${newProducts.length} new products`);
      ...
      console.log(error)
      const errorMessage = error instanceof Error ? error.message : JSON.stringify(error.message);
      const errorStack = error instanceof Error ? error.stack : undefined;

      logger.error(`Error importing products at page ${page}: ${errorMessage}\n${errorStack || ""}`);
  ...
  logger.info(`Product import completed. Total products imported: ${totalImported}`);
}
