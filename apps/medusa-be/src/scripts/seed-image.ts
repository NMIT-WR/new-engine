...
export default async function seedImages({ container }: ExecArgs) {
    ...
    try {
      ...
      for (const [productName, filePaths] of Object.entries(productImageMap)) {
        ...
        try {
          ...
          logger.info(
            `Upload successful for ${productName}. Files uploaded: ${result.map((f) => f.url).join(', ')}`
          );
      ...
      logger.info(
        `All products processed successfully. Products: ${Object.keys(results).join(', ')}. Total files: ${Object.values(
          results
  ...
  async function seedImages(container: MedusaContainer) {
    ...
    try {
      logger.info(
        `Starting image upload process. Products: ${Object.keys(productImageMap).length}, Files: ${Object.values(
          productImageMap
        ).reduce((acc, files) => acc + files.length, 0)}`
      );
      ...
      logger.info(
        `Image upload completed successfully. Products processed: ${Object.keys(result).join(', ')}`
      );
