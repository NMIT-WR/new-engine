...
const PREFETCH_BEHAVIOR = {
  ...
  /**
   * Batch configuration
   * Controls how many items to prefetch at once
   */
  BATCH: {
    /**
     * Max concurrent prefetch requests
     * Higher = faster initial load, more network pressure
     */
    MAX_CONCURRENT: 3,

    /**
     * Number of root categories to prefetch
     * Currently matches CATEGORY_MAP size
     */
    ROOT_CATEGORIES_SIZE: 8,
  },

  /**
   * Guard configuration
   * Controls prefetch frequency and deduplication
   */
  GUARDS: {
    /**
     * Use single-fetch guards (hasPrefetched refs)
     * Prevents duplicate prefetch on re-renders
     */
    USE_SINGLE_FETCH: true,

    /**
     * Enable scoped cancellation with prefetchedBy meta
     * Cancels prefetch when user navigates away
     */
    USE_SCOPED_CANCELLATION: true,
  },
} as const
