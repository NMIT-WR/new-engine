...
export function isDateString(value: string | Date | null | undefined | unknown): boolean {
    ...
    if (value instanceof Date || (typeof value === 'object' && isPureDateObject(value))) return true;
...
export function parseDrizzleMessage(message: string | undefined): DrizzleResult {
    ...
    if (m.length > 1_000_000) return { sql: m.slice(0, 10_000).trim(), params: [] };

    const [, sql, paramsString] = m.match(/^(.+?)(?:\s+--\sparams:\s+(.+))?$/) || [];
