import { normalizeString, parseDrizzleMessage, formatSQL, isDateString , safeEscapeSqlValue} from './utils';
...
function formatQuery(query: string, params: unknown[]): string {
    return params.length > 0
        ? params
            .map((value, i) => ({pattern: new RegExp(`\\s\\$${i + 1}`, 'i'), value: ` ${safeEscapeSqlValue(value)}`}))
            .reduce((currentQuery, {pattern, value}) => currentQuery.replace(pattern, value), query)
        : query;
}
...
// @ts-ignore
export const db = drizzle(process.env.DATABASE_URL, { logger, schema });
        ...
        (row): T =>
            Object.fromEntries(
                Object.entries(row).map(([key, value]) => [
                    key,
                    value && typeof value === 'string' && isDateString(value) ? new Date(value) : value,
                ])
            ) as T
    );
}

export async function sqlRawSingle<T = object>(sql: SQL<T>): Promise<T | undefined> {
    return (await sqlRaw(sql))[0];
}

export async function sqlRawSingleForce<T = object>(sql: SQL<T>, errorMessage?: string): Promise<T> {
...
export async function sqlRawCount(sql: SQL<unknown>): Promise<number> {
    return Number((await sqlRaw<{ count?: number }>(sql as SQL<{ count?: number }>))[0]?.count || 0);
}
