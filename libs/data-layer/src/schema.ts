import { pgTable, serial, text, integer, varchar, numeric, timestamp, uniqueIndex, index } from 'drizzle-orm/pg-core';
...
export const categories = pgTable(
    ...
    (table) => [
        index('categories_collection_id_idx').on(table.collectionId)
    ]
);
...
export const subcollections = pgTable(
    ...
    (table) => [
        index('subcollections_category_slug_idx').on(table.categorySlug)
    ]
);
...
export const subcategories = pgTable(
    'subcategories',
    {
        slug: text().primaryKey().notNull(),
        name: text().notNull(),
        subcollectionId: integer('subcollection_id').notNull().references(() => subcollections.id),
        imageUrl: text('image_url')
    },
    (table) => [
...
export const products = pgTable(
    ...
    (table) => [
        ...
        index('name_trgm_index').using('gin', (table) => table.name.op('gin_trgm_ops'))
    ]
);
