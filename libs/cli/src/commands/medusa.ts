...
const medusaContainerName = process.env.MEDUSA_CONTAINER_NAME ?? "wr_medusa_be"
...
export const medusaMinioInit = Command.make("medusa-minio-init", {}, () =>
  Effect.gen(function* () {
    ...
    const aliasExists = aliasList
      ...
      .map((line) => line.trim())
      .filter(Boolean)
      .some((line) => line.split(whitespaceRegex)[0] === "local")
    const minioEnvArgs = [
      "-e",
    ...
    yield* run("docker", ["cp", metadataZip, `${minioContainerName}:.`], {
      cwd: repoRoot,
    })
    yield* run(
      "docker",
