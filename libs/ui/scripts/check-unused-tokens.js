...
const NAMESPACE_TO_UTILITIES = {
  'color': ['bg', 'text', 'border', 'ring', 'fill', 'stroke', 'from', 'to', 'via', 'decoration', 'accent', 'caret', 'divide', 'outline', 'shadow', 'ring-offset'],
  'spacing': ['p', 'px', 'py', 'pt', 'pb', 'pl', 'pr', 'm', 'mx', 'my', 'mt', 'mb', 'ml', 'mr', 'gap', 'space-x', 'space-y', 'w', 'h', 'max-w', 'max-h', 'min-w', 'min-h', 'size', 'basis', 'inset', 'top', 'right', 'bottom', 'left'],
  'text': ['text'],
  'font-weight': ['font'],
  'font': ['font'],
  'border': ['border'],
...
function createSearchPatterns(utilities) {
  return utilities.map(utility => {
    ...
    return [
      ...
      new RegExp(`(?:hover|focus|focus-visible|active|disabled|group-hover|peer-focus|peer-disabled|group-disabled|placeholder):[^\\s"'\`]*${escaped}`),
      // Responsive prefixes
...
function isTokenUsed(token, allTokens) {
  ...
  for (const dir of SEARCH_DIRS) {
    ...
    for (const file of files) {
      ...
      const specialSyntaxCheck = content.includes(`(length:${token})`) || 
      ...
      if (specialSyntaxCheck) {
        const specialMatches = [
          content.match(new RegExp(`\\(length:${token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\)`)),
          content.match(new RegExp(`\\(width:${token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\)`)),
          content.match(new RegExp(`\\(height:${token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\)`)),
          content.match(new RegExp(`\\(size:${token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\)`))
        ].filter(m => m !== null)
        ...
        if (specialMatches.length > 0) {
          ...
          return { used: true, location: file, type: 'tailwind-v4-syntax', match: match[0] }
        }
      }
      ...
      for (let i = 0; i < searchPatterns.length; i++) {
        ...
        if (pattern.test(content)) {
          ...
          return { used: true, location: file, type: 'utility-class', match: match[0] }
        }
      }
    }
  }
...
function generateReport(unusedTokens, allTokens) {
  ...
  Object.entries(byFile).forEach(([file, tokens]) => {
    ...
    tokens.forEach(({ token, line }) => {
      const namespace = getTokenNamespace(token)
      const possibleUtils = generatePossibleUtilities(token).slice(0, 5).join(', ')
...
async function main() {
  ...
  const debugToken = args.find(arg => arg.startsWith('--token='))?.split('=')[1]
  ...
  for (const [token, info] of allTokens) {
    ...
    if (debugMode && (!debugToken || token === debugToken)) {
      ...
      console.log(`Possible utilities: ${utilities.slice(0, 10).join(', ')}${utilities.length > 10 ? '...' : ''}`)
    }
  ...
  if (unusedTokens.length === 0) {
    ...
    console.log(`   Unused tokens: ${unusedTokens.length} (${((unusedTokens.length / allTokens.size) * 100).toFixed(1)}%)`)
    console.log(`   Used tokens: ${allTokens.size - unusedTokens.length} (${(((allTokens.size - unusedTokens.length) / allTokens.size) * 100).toFixed(1)}%)`)
  }
